...
function editMenu(menuId) {
	$.ajax({
		...
		success : function(data) {
			if (data.status == "success") {
				var obj = data.data;
				reloadMenuModel("编辑菜单", obj.menu_id, obj.parent_id,
						obj.menu_name, obj.menu_url, obj.menu_type,
						obj.menu_icon, obj.sort_num);
...
function showSubMeunList(data, el, parentId) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		var subm = "<tr class='submenu"
				...
				+ submenu.menu_ORDER
				+ "</td>"
				+ "<td><span class='btn btn-xs btn-info' onclick='editMenu("
				+ submenu.menu_ID
				+ ")'><i class='fa fa-edit'></i> 修改</span> <span class='btn btn-xs btn-danger' onclick='delMenu("
				+ submenu.menu_ID
				+ ")'><i class='fa fa-trash-o'></i> 删除</span></td>" + "</tr>"
		subStr += subm;
	}
	el.after(subStr);
}
...
// 加载模态框的内容
function reloadMenuModel(title, menu_id, parent_id, menu_name, menu_url,
		menu_type, menu_icon, sort_num) {
